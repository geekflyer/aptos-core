...
def main():
    ...
    (runner_no, runner_cnt) = (int(sys.argv[1]), int(sys.argv[2])) if len(sys.argv) > 2 else (None, None)
    # by default we only run one job
    if runner_no is None:
        runner_no = 0
        runner_cnt = 1

    assert runner_no >= 0 and runner_no < runner_cnt, "runner_no must be between 0 and runner_cnt"
